...
//  HAL_GPIO_WritePin(GPIOE, LED1_RED_Pin|MEMS_LED_Pin|LCD_BL_CTRL_Pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(GPIOE, LED1_RED_Pin|MEMS_LED_Pin|LCD_BL_CTRL_Pin, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(GPIOE, LED1_RED_Pin, GPIO_PIN_SET);
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  char tt[] = "qq\r\n";
	  HAL_UART_Transmit(&huart6,(uint8_t*)tt,3,100);
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if(HAL_I2C_Slave_Receive(&hi2c2, (uint8_t *)aRxBuffer, RXBUFFERSIZE, 10000) != HAL_OK)
	   {
	     ...
	     //Error_Handler();
		  ...
		  if(RXBUFFERSIZE == hi2c2.XferSize)
			  continue;
	   }
	  ...
	  char ee[] = "ok";
	  char text[256];
	  sprintf(text,"Receive size:%d\n\r",RXBUFFERSIZE-hi2c2.XferSize);
	  HAL_UART_Transmit(&huart6,(uint8_t*)text,15,100);
	  HAL_UART_Transmit(&huart6,(uint8_t*)aRxBuffer,RXBUFFERSIZE-hi2c2.XferSize,100);
...
#endif
		char hh[] = "ok2";
		HAL_UART_Transmit(&huart6,(uint8_t*)hh,3,100);
